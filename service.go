...
import (
	"bitbucket.org/libertywireless/user-service/logging"
	"context"
...
func (c *userService) InsertUserIntoDatabase(ctx context.Context, user *User) (string, bool, int, string) {
	...
	if user.UserID == "" {
		logger.Errorf("user_id cannot be empty")
	...
	if user.Name == "" {
		logger.Errorf("name cannot be empty")
	...
	if duplicateUser.UserID != "" {
		logger.Errorf(`User with user_id: ` + duplicateUser.UserID + ` already exists in the database`)
	...
	if !ok {
		logger.Errorf("failed to convert inserted id into primitive object")
	...
	logger.Debugf(`Successfully inserted: ` + user.UserID + ` into database`)
...
func (c *userService) GetUserFromDatabase(ctx context.Context, userId string) (*User, bool, int, string) {
	...
	logger.Debugf(`Successfully got user: ` + user.UserID + ` from database`)
	return user, true, 0, ""
...
func (c *userService) GetAllUsersFromDatabase(ctx context.Context) ([]User, bool, int, string) {
	...
	//find all users from datasbase with empty filter
...
func (c *userService) DeleteUserFromDatabase(ctx context.Context, userId string) (string, bool, int, string) {
	...
	//if something went wrong during delete, reutrn 500 error
	if deleteErr != nil {
		logger.Errorf(deleteErr.Error())
		return "", false, http.StatusInternalServerError, deleteErr.Error()

	}
	//if user does not exist in database, return 400 error
	if resp.DeletedCount == 0 {
		logger.Errorf(`User with user_id: ` + userId + ` does not exist in the database`)
	...
	logger.Debugf(`Successfully deleted user: ` + userId + ` from database`)
...
func (c *userService) UpdateUserFromDatabase(ctx context.Context, user *User, userId string) (string, bool, int, string) {
	...
	//if usewr is trying to update user_id, return 400 error as it is not allowed
	if user.UserID != userId {
		return "", false, http.StatusBadRequest, "mismatching user_id/user_id cannot be updated"
	}
	...
	logger.Debugf(`Successfully updated user: ` + userId + ` from database`)
	return userId, true, 0, ""
